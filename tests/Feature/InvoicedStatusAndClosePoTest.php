 ...
 */
function purchaseOrderWithReceipts(User $user, array $receivedQtys, string $status = PurchaseOrder::STATUS_PARTIALLY_RECEIVED): array
{
    ...
    $goodsReceives = collect($receivedQtys)->values()->map(function (int $qty, int $index) use ($purchaseOrder, $purchaseOrderItem, $supplier, $product, $warehouse, $unit, $user) {
        $goodsReceive = GoodsReceive::create([
            'code' => 'GR-00'.($index + 1), 'date' => now()->toDateString(), 'supplier_id' => $supplier->id,
