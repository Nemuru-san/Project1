                                    ...
                                    @else
                                        <ul class="py-1 text-base text-gray-700 dark:text-gray-200">
                                            @if ($adjustment->status === 'draft')
                                                <li>
                                                    <button wire:click="confirmApprove({{ $adjustment->id }})" @disabled(! auth()->user()?->hasPermission('inventory.transaction.adjustment-in.approve'))
                                                        @click="open = false"
                                        ...
                                        <div class="py-1">
                                            @if ($adjustment->status === 'draft')
                                                <button wire:click="confirmDelete({{ $adjustment->id }})" @disabled(! auth()->user()?->hasPermission('inventory.transaction.adjustment-in.delete'))
                                                    @click="open = false"
        ...
        <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button wire:click="approve" wire:loading.attr="disabled" @disabled(! auth()->user()?->hasPermission('inventory.transaction.adjustment-in.approve'))
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-xl w-full max-w-md p-6">
                ...
                <div class="flex justify-end gap-2 mt-6">
                    ...
                    <button wire:click="delete" @disabled(! auth()->user()?->hasPermission('inventory.transaction.adjustment-in.delete'))
