<div x-data="{ toastMsg: '', toastType: '' }"
    ...
    <x-filter.card title="Filter Kategori Produk" description="Temukan kategori berdasarkan kode atau nama.">
        <x-slot:actions>
            <x-filter.add-button wire:click="openCreate">Tambah Kategori</x-filter.add-button>
        </x-slot:actions>
        <x-filter.search placeholder="Cari kode atau nama..." />
                            ...
                            <div class="inline-block" x-data="{
                                ...
                                <div x-show="open" x-cloak :style="`position: fixed; top: ${top}px; left: ${left}px;`"
                                    ...
                                    @else
                                        ...
                                        <div class="py-1">
                                            <button wire:click="confirmDelete({{ $cat->id }})" @disabled(! auth()->user()?->isSuperAdmin())
                                                @click="open = false"
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl w-full max-w-lg mx-auto max-h-[min(80vh,calc(100dvh-2rem))] flex flex-col overflow-hidden"
                ...
                <div
                    ...
                    <h3 class="text-lg font-semibold dark:text-white">
                        {{ $editingId ? 'Ubah' : 'Tambah' }} Kategori Produk
                    </h3>
