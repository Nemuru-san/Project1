<div x-data="{ toastMsg: '', toastType: '', showAddProductModal: false, rows: [
            { id: 2, uom: 'per-kotak', qty: '2', retail_price: '10', barcode: '' },
            { id: 3, uom: 'per-pak', qty: '3', retail_price: '15', barcode: '' }
        ], nextRowId: 4, addRow() { this.rows.push({ id: this.nextRowId++, uom: 'per-kotak', qty: '', retail_price: '', barcode: '' }) }, removeRow(id) { this.rows = this.rows.filter((row) => row.id !== id) } }"
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-xl w-full max-w-7xl mx-auto h-[90vh] flex flex-col overflow-hidden"
                    ...
                    <div class="flex-1 overflow-y-auto px-8 py-6">
                            ...
                            <div class="grid gap-5 sm:grid-cols-2 sm:gap-x-12 sm:gap-y-6">
                                <div class="w-full">
                                    <label for="product_code"
                                        class="block mb-3 text-base font-medium text-gray-900 dark:text-white">Product Code</label>
                                    <input type="text" name="product_code" id="product_code"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-zinc-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                        placeholder="Masukkan kode produk" required>
                                </div>

                                <div class="w-full">
                                    <label for="product_name"
                                        class="block mb-3 text-base font-medium text-gray-900 dark:text-white">Product Name</label>
                                ...
                                <div class="w-full">
                                    <label for="product_category"
                                        class="block mb-3 text-base font-medium text-gray-900 dark:text-white">Product Category</label>
                                    <select id="product_category" name="product_category"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-zinc-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                        required>
                                        <option value="">-- Pilih Kategori Produk --</option>
                                        <option value="sedotan">Sedotan</option>
                                        <option value="sumpit">Sumpit</option>
                                        <option value="tusuk-gigi">Tusuk Gigi</option>
                                    </select>
                                </div>

                                <div class="w-full">
                                    <label for="product_brand"
                                        class="block mb-3 text-base font-medium text-gray-900 dark:text-white">Product Brand</label>
                        ...
                        <div class="mt-12">
                            ...
                            <div class="overflow-x-auto max-w-8xl mx-auto px-0">
                                <table class="w-full table-fixed text-sm sm:text-base text-left text-gray-900 dark:text-white my-2 whitespace-normal border-collapse border border-gray-300 dark:border-zinc-600">
                                    <thead class="text-sm sm:text-base font-bold text-gray-900 uppercase bg-gray-200 dark:bg-zinc-700 dark:text-white">
                                        <tr>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-2 py-2 text-center w-14">
                                                <button type="button" @click.prevent="addRow()" class="inline-flex items-center justify-center w-9 h-9 rounded-sm bg-blue-600 text-white hover:bg-blue-700">
                                                    <span class="text-lg font-semibold">+</span>
                                                </button>
                                            </th>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-2 py-2 text-sm w-14">No</th>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm w-36">UOM</th>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm w-32">Kuantitas</th>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm w-48">Retail Price</th>
                                            <th scope="col" class="border border-gray-300 dark:border-zinc-600 px-3 py-2 text-sm w-60">Barcode</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <template x-for="(row, index) in rows" :key="row.id">
                                            <tr class="hover:bg-gray-100 dark:hover:bg-zinc-800 text-sm">
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3 text-center text-gray-500 dark:text-gray-400">
                                                    <button type="button" @click.prevent="removeRow(row.id)" class="text-red-500 hover:text-red-700">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                        </svg>
                                                    </button>
                                                </td>
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3 font-medium text-gray-900 dark:text-white" x-text="index + 1"></td>
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3">
                                                    <select x-model="row.uom"
                                                        ...
                                                        <option value="per-kotak">Per Kotak</option>
                                                        <option value="per-pak">Per Pak</option>
                                                        <option value="per-pcs">Per PCS</option>
                                                    </select>
                                                </td>
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3">
                                                    <input x-model="row.qty" type="text" placeholder="Kuantitas"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-zinc-800 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                                </td>
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3">
                                                    <input x-model="row.retail_price" type="text" placeholder="Retail Price"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2 dark:bg-zinc-800 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                                </td>
                                                <td class="border border-gray-300 dark:border-zinc-600 px-4 py-3">
                                                    <input x-model="row.barcode" type="text" placeholder="Barcode" disabled
                                                        class="bg-gray-100 border border-gray-300 text-gray-900 text-xs rounded-lg block w-full p-2 dark:bg-zinc-700 dark:border-gray-600 dark:text-gray-400 cursor-not-allowed">
                                                </td>
                                            </tr>
                                        </template>
                                    </tbody>
