<div>
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        @foreach ([['Jumlah Faktur', $summary['count']], ['Total Faktur', 'Rp ' . number_format($summary['total'], 0, ',', '.')], ['Sudah Dibayar', 'Rp ' . number_format($summary['paid'], 0, ',', '.')], ['Sisa Piutang', 'Rp ' . number_format($summary['outstanding'], 0, ',', '.')], ['Sudah Jatuh Tempo', 'Rp ' . number_format($summary['overdue'], 0, ',', '.')]] as $index => [$label, $value])
            <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <p
                    class="mt-2 text-xl font-bold {{ $index === 4 ? 'text-red-600' : ($index === 3 ? 'text-amber-600' : 'text-gray-900 dark:text-white') }}">
                    {{ $value }}</p>
            </div>
        @endforeach
    </div>
    <x-filter.card title="Faktur Penjualan Belum Lunas" description="Faktur terkonfirmasi dengan sisa tagihan lebih dari nol.">
        <x-filter.search placeholder="Cari faktur / SO / pelanggan" />
        <x-filter.select model="customerFilter" label="Pelanggan">
        ...
        <x-filter.per-page :show-reset="filled($search) || filled($customerFilter) || filled($paymentStatusFilter) || filled($dueFilter) || filled($dateFrom) || filled($dateTo)" />
    </x-filter.card>

    <div class="mt-5 overflow-x-auto rounded-xl border border-gray-200 dark:border-zinc-700">
        <table class="w-full min-w-[1300px] text-left text-sm text-gray-600 dark:text-gray-300">
            <thead class="bg-gray-100 text-xs uppercase text-gray-700 dark:bg-zinc-800 dark:text-gray-200">
                <tr>
                    <th class="px-4 py-3">No.</th>
                    <th wire:click="sortBy('invoice_no')" class="cursor-pointer px-4 py-3">Nomor Faktur</th>
                    <th class="px-4 py-3">Sales Order</th>
                    <th wire:click="sortBy('invoice_date')" class="cursor-pointer px-4 py-3">Tanggal</th>
                    ...
                    <th wire:click="sortBy('amount_due')" class="cursor-pointer px-4 py-3 text-right">Sisa</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-zinc-700 dark:bg-zinc-900">
                @forelse($invoices as $index => $invoice)
                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                        ...
                        <td class="px-4 py-3 font-mono">{{ $invoice->salesOrder?->order_no }}</td>
                        ...
                        <td class="px-4 py-3 text-right">Rp {{ number_format($invoice->dp_amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right text-green-600">Rp
                            {{ number_format($invoice->paid_amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-amber-600">Rp
                            {{ number_format($invoice->amount_due, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">{{ $invoice->age_days }} hari</td>
                        <td class="px-4 py-3 text-center">
                            @if ($invoice->overdue_days > 0)
                            <span class="font-semibold text-red-600">{{ $invoice->overdue_days }} hari</span>@else-
                            @endif
                        </td>
                        <td class="px-4 py-3"><span
                                class="rounded-full px-2.5 py-1 text-xs {{ $invoice->paid_amount > 0 ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700' }}">{{ $invoice->payment_label }}</span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                    ...
                                    @click.outside="open=false" class="cursor-pointer p-0.5 text-gray-500"><svg
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    class="z-50 w-52 rounded bg-white py-1 text-left shadow dark:bg-gray-700"><a
                                        href="{{ route('sales.transaction.salesInvoice.print', $invoice->id) }}"
                                        target="_blank"
                                        class="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600"><svg
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M6 9V3h12v6M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2M6 14h12v7H6v-7" />
                                        </svg>Cetak Faktur</a><a
                                        href="{{ route('finance.transaction.ar-payment', ['invoice' => $invoice->id]) }}"
                                        wire:navigate
                                        class="flex items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-green-600 hover:text-white"><svg
                                            class="h-5 w-5" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 6v12m6-6H6" />
                                        </svg>Buat Pembayaran</a></div>
                            </div>
                        </td>
                    </tr>
                @empty<tr>
                        <td colspan="14" class="px-4 py-10 text-center text-gray-400">Tidak ada Faktur Penjualan
                            yang belum lunas.</td>
                    </tr>
                @endforelse
            </tbody>
