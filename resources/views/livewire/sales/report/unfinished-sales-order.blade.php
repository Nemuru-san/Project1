<div>
    ...
    <x-filter.card title="Daftar SO Belum Selesai" description="Sales Order terkonfirmasi yang masih memiliki qty belum dikirim.">
        <x-filter.search placeholder="Cari SO / pelanggan" />
        ...
        <x-filter.select model="deliveryFilter" label="Status kirim">
        ...
        <x-filter.date-range label="Rentang tanggal SO" />
        <x-filter.per-page :show-reset="filled($search) || filled($customerFilter) || filled($warehouseFilter) || filled($deliveryFilter) || filled($dateFrom) || filled($dateTo)" />
    </x-filter.card>
