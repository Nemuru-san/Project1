    ...
    <div class="my-4 flex flex-col gap-3 dark:bg-zinc-900">
        <div class="flex w-full flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            <div class="relative w-full sm:w-72">
                ...
                <input wire:model.live.debounce.300ms="search" type="search" placeholder="Cari nomor, customer, salesman..."
            ...
            <label class="flex cursor-pointer items-center gap-2 whitespace-nowrap rounded-lg border border-gray-600 px-3 py-2.5 text-sm text-gray-700 dark:text-gray-300">
                <input wire:model.live="showTrashed" type="checkbox" class="h-4 w-4 rounded border-gray-600 text-blue-600 dark:bg-zinc-800">
                Tampilkan Terhapus
            </label>
        ...
        <table class="w-full text-left text-sm text-gray-600 dark:text-gray-300">
            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-zinc-700 dark:bg-zinc-900">
                @forelse ($salesCanvases as $canvas)
                    <tr wire:key="canvas-{{ $canvas->id }}" class="hover:bg-gray-50 dark:hover:bg-zinc-800 {{ $canvas->trashed() ? 'opacity-60' : '' }}">
                        <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $canvas->canvas_no }}</td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right font-medium">Rp {{ number_format($canvas->grand_total, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            @if ($canvas->trashed())
                            ...
                            @elseif ($canvas->status !== 'draft')
                                <span class="rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-300">Diproses</span>
                            @else
                                <span class="rounded-full bg-yellow-100 px-2.5 py-1 text-xs text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300">Draf</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect(); this.top = r.bottom + 6; this.left = Math.max(8, r.right - 224); this.open = !this.open } }">
                                <button type="button" @click="toggle($el)" @click.outside="open = false" aria-label="Buka aksi penjualan kanvas"
                                ...
                                <div x-cloak x-show="open" :style="`position: fixed; top: ${top}px; left: ${left}px;`"
                                    ...
                                    <ul class="whitespace-nowrap py-1 text-sm text-gray-700 dark:text-gray-200">
                                        <li>
                                            <button type="button" wire:click="openDetail({{ $canvas->id }})" @click="open = false" class="flex w-full cursor-pointer items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">
                                        ...
                                        @if (! $canvas->trashed() && $canvas->status === 'draft')
                                            <li>
                                                <button type="button" wire:click="openEdit({{ $canvas->id }})" @click="open = false" class="flex w-full cursor-pointer items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600">
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-10 text-center text-gray-400">Belum ada transaksi penjualan kanvas.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="fixed inset-0 z-40 flex items-start justify-center overflow-hidden bg-black/50 p-4 backdrop-blur-sm">
            <div class="mx-auto flex h-[80vh] max-h-[calc(100dvh-2rem)] w-full max-w-full flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-zinc-800">
                <div class="flex shrink-0 items-center justify-between border-b border-gray-200 bg-zinc-50 px-8 py-6 dark:border-zinc-700 dark:bg-zinc-900">
                    <h3 class="text-lg font-semibold dark:text-white">{{ $editingId ? 'Ubah Penjualan Kanvas' : 'Tambah Penjualan Kanvas' }}</h3>
                    <button type="button" wire:click="$set('showModal', false)" class="cursor-pointer text-gray-400 hover:text-white">
                ...
                <form wire:submit="save" x-on:keydown.enter="if ($event.target.tagName === 'INPUT') $event.preventDefault()" class="flex min-h-0 flex-1 flex-col">
                    <div class="min-h-0 flex-1 overflow-y-auto px-8 py-6">
                        <div class="grid gap-5 sm:grid-cols-2 sm:gap-x-12 sm:gap-y-6">
                            <div class="w-full">
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Nomor Canvas</label>
                                <input wire:model="canvasNo" type="text" readonly class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                            </div>
                            <div class="w-full">
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Tanggal</label>
                                <input wire:model="date" type="date" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-600 focus:ring-primary-600 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                @error('date') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>
                            <div class="w-full">
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Salesman</label>
                                @if (auth()->user()->isSuperAdmin())
                                    <select wire:model.live="salesmanId" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                ...
                                @else
                                    <input type="text" value="{{ $selectedSalesman ? $selectedSalesman->code.' - '.$selectedSalesman->name : '' }}" readonly class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                                    <p class="mt-1 text-xs text-gray-400">Otomatis mengikuti salesman yang sedang login.</p>
                                @endif
                                @error('salesmanId') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>
                            <div class="w-full">
                                ...
                                <label class="inline-flex cursor-pointer items-center">
                                    ...
                                    <span class="relative mx-3">
                                        ...
                                        <span class="block h-5 w-9 rounded-full bg-red-200 after:absolute after:start-0.5 after:top-0.5 after:h-4 after:w-4 after:rounded-full after:bg-red-500 after:transition-all after:content-[''] peer-checked:bg-primary-600 peer-checked:after:translate-x-full"></span>
                                    </span>
                                    <span class="select-none text-base font-medium text-gray-600 dark:text-gray-400">Ya</span>
                                </label>
                            </div>
                            <div class="w-full">
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Customer</label>
                                <select wire:model.live="customerId" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                    <option value="">-- Pilih Customer --</option>
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id }}">{{ $customer->code }} - {{ $customer->name }}</option>
                                    @endforeach
                                </select>
                                @error('customerId') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>
                            <div class="w-full">
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Alamat Pengiriman</label>
                                <select wire:model="customerAddressId" @disabled(! $customerId) class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 disabled:opacity-50 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                    <option value="">Tanpa alamat pengiriman</option>
                                    @foreach ($customerAddresses as $address)
                                        <option value="{{ $address->id }}">{{ $address->code }} - {{ $address->label }}</option>
                                    @endforeach
                                </select>
                                @error('customerAddressId') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div class="mt-6 w-full">
                            <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Catatan</label>
                            <textarea wire:model="notes" rows="3" placeholder="Masukkan catatan atau keterangan tambahan..." class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-600 focus:ring-primary-600 dark:border-gray-600 dark:bg-zinc-800 dark:text-white"></textarea>
                        ...
                        <div class="mt-12">
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full min-w-max border-collapse border border-gray-300 text-left text-sm text-gray-900 whitespace-nowrap dark:border-zinc-600 dark:text-white">
                                    <thead class="bg-gray-200 text-sm font-bold text-gray-900 uppercase dark:bg-zinc-700 dark:text-white">
                                        <tr>
                                            <th class="w-14 border border-gray-300 px-4 py-3 text-center dark:border-zinc-600">
                                                <button wire:click="openProductPicker" type="button" class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-sm bg-blue-600 text-white hover:bg-blue-700">
                                                    <span class="text-xl font-semibold">+</span>
                                                </button>
                                            </th>
                                            <th class="border border-gray-300 px-4 py-3 text-sm dark:border-zinc-600">No.</th>
                                            <th class="border border-gray-300 px-4 py-3 text-sm dark:border-zinc-600">Kode Produk</th>
                                            ...
                                            <th class="border border-gray-300 px-4 py-3 text-sm dark:border-zinc-600">Sub Total</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse ($items as $index => $item)
                                            <tr wire:key="canvas-item-{{ $item['product_id'] }}" class="text-sm hover:bg-gray-100 dark:hover:bg-zinc-800">
                                                <td class="border border-gray-300 px-4 py-3 text-center dark:border-zinc-600">
                                                    <button wire:click="removeItem({{ $index }})" type="button" class="cursor-pointer text-red-500 hover:text-red-700" aria-label="Hapus produk">
                                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                    </button>
                                                </td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">{{ $index + 1 }}</td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">{{ $item['sku'] }}</td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">{{ $item['name'] }}</td>
                                                ...
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">
                                                    <select wire:model.live="items.{{ $index }}.unit_id" class="block w-36 rounded-lg border border-gray-300 bg-gray-50 p-2 text-xs text-gray-900 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                                        @foreach ($item['unit_options'] as $unit)
                                                            <option value="{{ $unit['unit_id'] }}">{{ $unit['unit_name'] }} (x{{ $unit['conversion'] }})</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">
                                                ...
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">
                                                    <input type="text" inputmode="numeric" autocomplete="off"
                                                        x-data="{ display: '{{ number_format($item['qty'] ?? 0, 0, ',', '.') }}' }"
                                                        x-model="display"
                                                        @input="
                                                    ...
                                                    @error("items.$index.qty") <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                                                </td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">
                                                    <input type="text" inputmode="numeric" autocomplete="off"
                                                    ...
                                                    @error("items.$index.unit_price") <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                                                </td>
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600">
                                                    <input type="text" inputmode="numeric" autocomplete="off"
                                                        x-data="{ display: '{{ number_format($item['discount_amount'] ?? 0, 0, ',', '.') }}' }"
                                                        x-model="display"
                                                        @input="
                                                            let raw = display.replace(/\./g, '').replace(/\D/g, '');
                                                            display = raw === '' ? '' : Number(raw).toLocaleString('id-ID');
                                                        ...
                                                        "
                                                        class="block w-28 rounded-lg border border-gray-300 bg-gray-50 p-2 text-xs text-gray-900 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                                    @error("items.$index.discount_amount") <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                                                </td>
                                        ...
                                        @empty
                                            <tr><td colspan="11" class="border border-gray-300 px-4 py-8 text-center text-sm text-gray-400 dark:border-zinc-600">Belum ada produk. Klik tombol <strong>+</strong> untuk menambahkan.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="mt-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-zinc-800">
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                <div>
                                    ...
                                    <input type="text" value="Rp {{ number_format($totals['subtotal'], 0, ',', '.') }}" disabled class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                                </div>
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Total Diskon</label>
                                    <input type="text" value="Rp {{ number_format($totals['discount'], 0, ',', '.') }}" disabled class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                                </div>
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">PPN (11%)</label>
                                    <input type="text" value="Rp {{ number_format($totals['tax'], 0, ',', '.') }}" disabled class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                                </div>
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Neto</label>
                                    <input type="text" value="Rp {{ number_format($totals['grand_total'], 0, ',', '.') }}" disabled class="block w-full cursor-not-allowed rounded-lg border border-gray-300 bg-gray-100 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-700 dark:text-gray-400">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex shrink-0 justify-end gap-2 border-t border-gray-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                        <button wire:click="$set('showModal', false)" type="button" class="cursor-pointer rounded-lg border border-gray-600 px-4 py-2 text-sm dark:text-gray-300 hover:bg-zinc-700">Batal</button>
                        <button type="submit" wire:loading.attr="disabled" wire:target="save" class="cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50">
                            <span wire:loading.remove wire:target="save">Simpan</span><span wire:loading wire:target="save">Menyimpan...</span>
                        </button>
                    </div>
                </form>
            </div>
    ...
    @if ($showProductModal)
        <div class="fixed inset-0 z-[60] flex items-start justify-center overflow-hidden bg-black/50 p-4 backdrop-blur-sm">
            <div class="flex h-[80vh] max-h-[calc(100dvh-2rem)] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-zinc-900">
                <div class="flex shrink-0 items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-zinc-700">
                    <div>
                    ...
                    <button wire:click="$set('showProductModal', false)" type="button" class="cursor-pointer text-gray-400 hover:text-white">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
                <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-6">
                    <div class="grid items-end gap-4 sm:grid-cols-[1fr_auto]">
                        <div class="relative">
                            <input wire:model.live.debounce.300ms="productSearch" type="text" placeholder="Cari produk (nama / kode)..." class="w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 pl-10 text-sm text-gray-900 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                            <svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.5 5.5a7.5 7.5 0 0 0 10.5 10.5Z" /></svg>
                        </div>
                    ...
                    <div class="overflow-x-auto rounded-xl border border-gray-300 dark:border-zinc-600">
                        <table class="w-full border-collapse text-left text-sm text-gray-900 dark:text-white">
                            <thead class="bg-gray-100 text-xs font-semibold uppercase dark:bg-zinc-800 dark:text-gray-200">
                                <tr>
                                    <th class="w-12 border border-gray-300 px-4 py-3 text-center dark:border-zinc-700">Pilih</th>
                    ...
                    <div class="flex justify-end gap-2 border-t border-gray-200 pt-4 dark:border-zinc-700">
                        <button wire:click="$set('showProductModal', false)" type="button" class="cursor-pointer rounded-lg border border-gray-300 px-4 py-2 text-sm hover:bg-gray-100 dark:border-zinc-600 dark:text-gray-300 dark:hover:bg-zinc-700">Batal</button>
                        <button wire:click="addSelectedProducts" type="button" class="cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700">Tambah Produk Terpilih</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div class="mx-4 w-full max-w-sm rounded-xl bg-white p-6 shadow-xl dark:bg-zinc-800">
                <div class="mb-4 flex items-center gap-3">
                    <div class="rounded-full bg-blue-900 p-2"><svg class="h-5 w-5 text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0-4-4m4 4-4 4M16 17H4m0 0 4 4m-4-4 4-4" /></svg></div>
                    <h3 class="text-base font-semibold dark:text-white">Jadikan Sales Order?</h3>
                </div>
                <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">Header dan seluruh produk akan disalin ke Sales Order. Penjualan kanvas ini akan dikunci dan statusnya berubah menjadi Sales Order.</p>
                <div class="flex justify-end gap-2">
                    <button wire:click="$set('showConvertModal', false)" class="cursor-pointer rounded-lg border border-gray-300 px-4 py-2 text-sm hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-zinc-700">Batal</button>
                    <button wire:click="convertToSalesOrder" wire:loading.attr="disabled" class="cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50"><span wire:loading.remove wire:target="convertToSalesOrder">Ya, Buat Sales Order</span><span wire:loading wire:target="convertToSalesOrder">Memproses...</span></button>
                </div>
            </div>
        </div>
    @endif
            ...
            <div class="flex max-h-[min(80vh,calc(100dvh-2rem))] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-zinc-900">
                ...
                <div class="min-h-0 flex-1 overflow-y-auto p-6">
                    <dl class="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                        ...
                        <div><dt class="text-gray-400">Salesman</dt><dd class="font-medium dark:text-white">{{ $selectedCanvas->salesman?->name }}</dd></div>
