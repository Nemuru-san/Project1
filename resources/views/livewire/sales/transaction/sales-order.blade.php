            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-zinc-700 dark:bg-zinc-900">
                @forelse ($salesOrders as $order)
                    <tr wire:key="sales-order-{{ $order->id }}"
                        class="hover:bg-gray-50 dark:hover:bg-zinc-800 {{ $order->trashed() ? 'opacity-60' : '' }}">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right text-green-600">Rp
                            {{ number_format($order->dp_amount, 0, ',', '.') }}</td>
                        <td class="whitespace-nowrap px-4 py-3 text-right font-medium">Rp
                            {{ number_format($order->amount_due, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            @if ($order->trashed())
                        ...
                        <td class="px-4 py-3">
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    ...
                                    @if ($order->trashed())
                                        <button wire:click="restore({{ $order->id }})" @click="open=false"
                                            @disabled(!auth()->user()?->isSuperAdmin())
                                            class="flex w-full cursor-pointer items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-green-600 hover:text-white disabled:cursor-not-allowed disabled:opacity-40">Pulihkan</button>
                                    @else
                                        <ul class="whitespace-nowrap py-1 text-sm dark:text-gray-200">
                                            <li><button wire:click="openDetail({{ $order->id }})"
                                                    @click="open=false"
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-4 py-10 text-center text-gray-400">Belum ada Sales Order.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
            ...
            <div
                ...
                <div
                    ...
                    <h3 class="text-lg font-semibold dark:text-white">
                        {{ $editingId ? 'Ubah Sales Order' : 'Tambah Sales Order' }}</h3>
                ...
                <form wire:submit="save" x-on:scan-focus.window="$nextTick(() => $refs.scanInput?.focus())"
                    ...
                    <div class="min-h-0 flex-1 touch-pan-y overflow-y-auto px-8 py-6"
                        ...
                        <div class="grid gap-5 sm:grid-cols-2 sm:gap-x-12 sm:gap-y-6">
                            ...
                            <div><label
                                    class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Tanggal</label><input
                            ...
                            <div><label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Sumber Sales Order</label><select wire:model.live="sourceType"
                                    ...
                                    <option value="sales_canvas">Sales Kanvas</option>
                            ...
                            @if ($sourceType === 'sales_canvas')
                                <div><label
                                        ...
                                        Sales Kanvas</label><select wire:model.live="salesCanvasId"
                                        @disabled($editingId)
                                        class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                        <option value="">-- Pilih Sales Kanvas --</option>
                                        @foreach ($salesCanvases as $salesCanvas)
                                            <option value="{{ $salesCanvas->id }}">{{ $salesCanvas->canvas_no }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('salesCanvasId')
                            ...
                            @elseif($sourceType === 'pre_order')
                                <div><label
                                        class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Nomor
                                        Pre Order</label><select wire:model.live="preOrderId"
                                        @disabled($editingId)
                                        class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-zinc-800 dark:text-white">
                                        <option value="">-- Pilih Pre Order --</option>
                                        @foreach ($preOrders as $preOrder)
                                            <option value="{{ $preOrder->id }}">{{ $preOrder->pre_order_no }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('preOrderId')
                            ...
                            @endif
                            <div><label
                                    class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Customer</label><select
                                ...
                                @enderror
                                @if ($creditSummary)
                                    <x-credit-summary :summary="$creditSummary" class="mt-3" />
                                @endif
                            </div>
                            <div>
                                <label class="mb-3 block text-base font-medium text-gray-900 dark:text-white">Pajak
                                    (PPN 11%)</label>
                                <label class="inline-flex cursor-pointer items-center"><span
                                        class="text-base text-gray-600 dark:text-gray-400">Tidak</span><span
                                        class="relative mx-3"><input wire:model.live="tax" type="checkbox"
                                            class="peer sr-only"><span
                                            class="block h-5 w-9 rounded-full bg-red-200 after:absolute after:start-0.5 after:top-0.5 after:h-4 after:w-4 after:rounded-full after:bg-red-500 after:transition-all after:content-[''] peer-checked:bg-primary-600 peer-checked:after:translate-x-full"></span></span><span
                                        class="text-base text-gray-600 dark:text-gray-400">Ya</span></label>
                            </div>
                                        ...
                                        <tr>
                                            <th
                                                ...
                                                <button wire:click="openProductPicker" type="button"
                                                    class="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-sm bg-blue-600 text-xl text-white hover:bg-blue-700">+</button>
                                            </th>
                                        ...
                                        @forelse($items as $index => $item)
                                            <tr wire:key="so-item-{{ $item['product_id'] }}-{{ $item['warehouse_id'] }}-{{ $index }}"
                                                ...
                                                <td class="border border-gray-300 px-4 py-3 dark:border-zinc-600"><span
                                                        class="rounded bg-blue-100 px-2.5 py-1 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">{{ $item['stock_available_display'] ?? number_format($item['stock_available'], 0, ',', '.') . ' ' . $item['base_unit_name'] }}</span>
                                                </td>
                        ...
                        @if($orderType === 'direct')
                            ...
                            <div class="mt-6 rounded-xl border border-emerald-300 bg-emerald-50 p-5 dark:border-emerald-800 dark:bg-emerald-950/30">
                                ...
                                @if($selectedCustomer)
                                    <div class="mb-4 grid gap-2 rounded-lg bg-white/70 p-3 text-sm md:grid-cols-2 lg:grid-cols-5 dark:bg-zinc-900/50 dark:text-gray-200">
                                        ...
                                        <div>Piutang Berjalan: <strong>Rp {{ number_format($creditSummary['outstanding'] ?? 0, 0, ',', '.') }}</strong></div>
                                        <div>Sisa Plafon: <strong class="{{ ($creditSummary['remaining'] ?? 0) < 0 ? 'text-red-600' : '' }}">{{ ($creditSummary['remaining'] ?? null) === null ? 'Tanpa batas' : 'Rp '.number_format($creditSummary['remaining'], 0, ',', '.') }}</strong></div>
            ...
            <div
                ...
                <div class="flex items-center justify-between border-b px-6 py-4 dark:border-zinc-700">
                    <div>
                        ...
                        class="cursor-pointer text-gray-400">&times;</button>
                </div>
                <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-6">
                        ...
                        <table class="w-full text-left text-sm dark:text-white">
                            ...
                            <tbody>
                                @forelse($products as $product)
                                    ...
                                    <tr wire:key="sales-order-product-picker-{{ $product->id }}"
                                        @click="$event.currentTarget.querySelector('input[type=checkbox]:not(:disabled)')?.click()"
                                        class="border-t dark:border-zinc-700 {{ $added ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800' }}">
                                        ...
                                        <td colspan="4" class="px-4 py-8 text-center text-gray-400">Produk tidak
                                            ditemukan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="flex justify-end gap-2 border-t pt-4 dark:border-zinc-700"><button
                            ...
                            class="rounded-lg border px-4 py-2 text-sm dark:text-white">Batal</button><button
                            wire:click="addSelectedProducts"
                            class="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">Tambah Produk Terpilih</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div
                ...
                <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-zinc-700">
                    <div>
                        <h3 class="text-lg font-semibold dark:text-white">Detail Sales Order</h3>
                        <p class="mt-0.5 font-mono text-sm text-gray-400">{{ $selectedOrder->order_no }}</p>
                    </div>
                ...
                <div class="min-h-0 flex-1 overflow-y-auto p-6">
                    <dl class="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                        <div>
                            ...
                            <dd class="font-medium dark:text-white">{{ $selectedOrder->date->format('d/m/Y') }}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <dd class="font-medium dark:text-white">{{ $selectedOrder->customer?->name ?? '-' }}</dd>
                        </div>
                    </dl>
                    ...
                    <div class="ml-auto mt-5 w-full max-w-sm space-y-2 text-sm">
                        <div class="flex justify-between"><span>Bruto</span><span>Rp
                                {{ number_format($selectedOrder->subtotal, 0, ',', '.') }}</span></div>
                        <div class="flex justify-between"><span>Diskon</span><span>Rp
                                {{ number_format($selectedOrder->discount_total, 0, ',', '.') }}</span></div>
                        <div class="flex justify-between"><span>PPN</span><span>Rp
                                {{ number_format($selectedOrder->tax_amount, 0, ',', '.') }}</span></div>
                        <div
                            class="flex justify-between border-t pt-2 text-base font-bold dark:border-zinc-700 dark:text-white">
                            <span>Neto</span><span>Rp
                                {{ number_format($selectedOrder->grand_total, 0, ',', '.') }}</span></div>
                        <div class="flex justify-between text-green-600"><span>DP Terpakai</span><span>- Rp
                                {{ number_format($selectedOrder->dp_amount, 0, ',', '.') }}</span></div>
                        <div
                            class="flex justify-between border-t pt-2 text-base font-bold dark:border-zinc-700 dark:text-white">
                            <span>Sisa Tagihan</span><span>Rp
                                {{ number_format($selectedOrder->amount_due, 0, ',', '.') }}</span></div>
                    </div>
                </div>
                <div class="flex justify-end border-t border-gray-200 px-6 py-4 dark:border-zinc-700"><button
                        ...
                        class="cursor-pointer rounded-lg bg-zinc-700 px-5 py-2 text-sm font-medium text-white hover:bg-zinc-600">Tutup</button>
                </div>
            </div>
        </div>
    @endif
        ...
        <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4">
            <div class="w-full max-w-md rounded-2xl bg-white p-6 dark:bg-zinc-800">
                <div class="mb-4 flex items-center gap-3">
                    ...
                    <h3 class="text-lg font-semibold dark:text-white">Konfirmasi Pesanan Penjualan?</h3>
                </div>
                <p class="mb-4 text-sm text-gray-400">Setelah dikonfirmasi, pesanan tidak dapat diubah dan dapat
                    digunakan untuk Surat Jalan serta Pembayaran Piutang.</p>
