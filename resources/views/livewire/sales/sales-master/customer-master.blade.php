                            ...
                            <div class="inline-block" x-data="{
                                ...
                                <div x-show="open" x-cloak :style="`position: fixed; top: ${top}px; left: ${left}px;`"
                                    ...
                                    <div class="py-1">
                                        ...
                                        @else
                                            <button type="button" wire:click="confirmDelete({{ $customer->id }})" @click="open = false" @disabled(! auth()->user()->isSuperAdmin())
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-zinc-900">
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <button type="button" wire:click="delete" wire:loading.attr="disabled" @disabled(! auth()->user()->isSuperAdmin()) class="cursor-pointer rounded-lg bg-red-600 px-4 py-2 text-sm text-white hover:bg-red-700 disabled:opacity-50">Hapus</button>
                </div>
            </div>
        </div>
    @endif
