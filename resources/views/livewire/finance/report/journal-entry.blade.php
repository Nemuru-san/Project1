<div x-data="{ toastMsg: '', toastType: '' }"
    ...
    <x-filter.card title="Filter Entri Jurnal" description="Temukan jurnal berdasarkan kata kunci, status, atau sumber transaksi.">
        <x-filter.search placeholder="Cari kode, sumber, deskripsi..." />
        ...
        <x-filter.select model="sourceFilter" label="Sumber">
            <option value="">Semua Sumber</option>
            @foreach ($sourceOptions as $source => $label)
                <option value="{{ $source }}">{{ $label }}</option>
            @endforeach
        </x-filter.select>
        <x-filter.per-page :show-reset="filled($search) || filled($statusFilter) || filled($sourceFilter)" />
