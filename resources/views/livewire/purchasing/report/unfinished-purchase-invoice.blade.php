<div>
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        @foreach ([['Jumlah Faktur', $summary['count']], ['Total Faktur', 'Rp ' . number_format($summary['total'], 0, ',', '.')], ['Sudah Dibayar', 'Rp ' . number_format($summary['paid'], 0, ',', '.')], ['Sisa Utang', 'Rp ' . number_format($summary['outstanding'], 0, ',', '.')], ['Sudah Jatuh Tempo', 'Rp ' . number_format($summary['overdue'], 0, ',', '.')]] as $index => [$label, $value])
        ...
        <table class="w-full min-w-[1350px] text-left text-sm text-gray-600 dark:text-gray-300">
            <thead class="bg-gray-100 text-xs uppercase text-gray-700 dark:bg-zinc-800 dark:text-gray-200">
                <tr>
                    ...
                    <th wire:click="sortBy('remaining_amount')" class="cursor-pointer px-4 py-3 text-right">Sisa</th>
                            ...
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    class="z-50 w-52 rounded bg-white py-1 text-left shadow dark:bg-gray-700"><a
                                        href="{{ route('purchases.transaction.purchase-invoice', ['invoice' => $invoice->id]) }}"
                                        wire:navigate
                                        ...
                                        </svg>Rincian Faktur</a><a
                                        href="{{ route('purchases.transaction.purchase-invoice.print', $invoice->id) }}"
                                        target="_blank"
                                        class="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600"><svg
                                        ...
                                        href="{{ route('finance.transaction.ap-payment', ['invoice' => $invoice->id]) }}"
                                        wire:navigate
