<div>
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ([['Jumlah PO', $summary['count'], 'text-blue-600'], ['Qty Dipesan', number_format($summary['ordered'], 0, ',', '.'), 'text-gray-900 dark:text-white'], ['Sudah Diterima', number_format($summary['received'], 0, ',', '.'), 'text-green-600'], ['Qty Belum Diterima', number_format($summary['outstanding'], 0, ',', '.'), 'text-amber-600']] as [$label, $value, $color])
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-zinc-700 dark:bg-zinc-900">
                @forelse($purchaseOrders as $index => $po)
                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                        ...
                        <td class="px-4 py-3 font-mono font-medium text-gray-900 dark:text-white">{{ $po->code }}
                        </td>
                        ...
                        <td class="px-4 py-3 text-right text-green-600">
                            {{ number_format($po->received_qty, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-amber-600">
                            {{ number_format($po->outstanding_qty, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right">Rp {{ number_format($po->nett, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">{{ $po->age_days }} hari</td>
                        <td class="px-4 py-3"><span
                                class="rounded-full px-2.5 py-1 text-xs {{ $po->received_qty > 0 ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700' }}">{{ $po->receipt_label }}</span>
                        </td>
                            ...
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    class="z-50 w-52 rounded bg-white py-1 text-left shadow dark:bg-gray-700"><a
                                        href="{{ route('purchases.transaction.purchase-order', ['order' => $po->id]) }}"
                                        wire:navigate
                                        ...
                                        href="{{ route('purchases.transaction.purchase-order.print', $po->id) }}"
                                        target="_blank"
                                        class="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-600"><svg
                                            class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        href="{{ route('purchases.transaction.good-receive', ['order' => $po->id]) }}"
                                        wire:navigate
                                        ...
                                        </svg>Penerimaan Barang</a></div>
                            </div>
                        </td>
                    </tr>
                @empty<tr>
                        <td colspan="11" class="px-4 py-10 text-center text-gray-400">Tidak ada PO yang belum
                            selesai.</td>
                    </tr>
                @endforelse
            </tbody>
