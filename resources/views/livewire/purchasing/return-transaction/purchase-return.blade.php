<div x-data="{ toastMsg: '', toastType: '' }"
    ...
    <x-filter.card title="Filter Retur Pembelian" description="Temukan retur berdasarkan kata kunci, status, atau rentang tanggal.">
        <x-slot:actions>
            <x-filter.add-button wire:click="openCreate" :disabled="! auth()->user()?->hasPermission('purchases.return.purchase-return')">Tambah Retur</x-filter.add-button>
        </x-slot:actions>
        <x-filter.search placeholder="Cari nomor retur / GR / supplier" />
        ...
        <table class="w-full min-w-[1050px] text-left text-sm text-gray-600 dark:text-gray-300">
            <thead class="bg-gray-100 text-xs uppercase dark:bg-zinc-800">
                <tr>
                    ...
                    <th class="px-4 py-3">Penerimaan Barang</th>
            ...
            <tbody class="divide-y dark:divide-zinc-700 dark:bg-zinc-900">
                @forelse($returns as $i=>$return)
                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                        ...
                        <td class="px-4 py-3">{{ $return->supplier?->name }}</td>
                        <td class="px-4 py-3 font-mono">{{ $return->goodsReceive?->code }}</td>
                        <td class="px-4 py-3 font-mono">{{ $return->purchaseOrder?->code }}</td>
                            ...
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    class="z-50 w-52 rounded bg-white py-1 text-left shadow dark:bg-gray-700"><button
                                        ...
                                        href="{{ route('purchases.return.purchase-return.print', $return->id) }}"
                                        target="_blank"
                                    ...
                                    @if ($return->status === 'Draft')
                                        ...
                                        @endif @if ($return->status === 'Confirmed' && !$return->returnInvoice)
                                            <a href="{{ route('purchases.return.purchase-return-invoice', ['return' => $return->id]) }}"
                                                wire:navigate
                ...
                @empty<tr>
                        <td colspan="10" class="px-4 py-10 text-center text-gray-400">Belum ada Retur Pembelian.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4">
            <div class="my-4 w-full max-w-5xl rounded-2xl bg-white shadow-2xl dark:bg-zinc-900">
                <div class="flex items-center justify-between border-b px-6 py-4 dark:border-zinc-700">
                    <div>
                        <h2 class="text-xl font-bold">Tambah Retur Pembelian</h2>
                        <p class="text-sm text-gray-400">Pilih Penerimaan Barang yang sudah diterima.</p>
                ...
                <div class="space-y-5 p-6">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div><label class="mb-1 block text-sm font-medium">Penerimaan Barang</label><select
                                wire:model.live="goodsReceiveId"
                                ...
                                @foreach ($goodsReceives as $gr)
                                    <option value="{{ $gr->id }}">{{ $gr->code }} —
                                        {{ $gr->supplier?->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <tbody class="divide-y dark:divide-zinc-700">
                                @forelse($items as $i=>$item)
                                    <tr>
                                        ...
                                        <td colspan="6" class="px-4 py-8 text-center text-gray-400">Pilih
                                            Penerimaan Barang terlebih dahulu.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4">
            <div class="my-4 w-full max-w-4xl rounded-2xl bg-white shadow-2xl dark:bg-zinc-900">
                <div class="flex justify-between border-b px-6 py-4 dark:border-zinc-700">
                    <div>
                        <h2 class="text-xl font-bold">Detail Retur Pembelian</h2>
                ...
                <div class="space-y-5 p-6">
                    <div class="grid gap-3 text-sm md:grid-cols-3">
                        <div><span class="text-gray-400">Supplier</span>
                            <p class="font-medium">{{ $selectedReturn->supplier?->name }}</p>
                        </div>
                        <div><span class="text-gray-400">Penerimaan Barang</span>
                            <p class="font-mono">{{ $selectedReturn->goodsReceive?->code }}</p>
                        </div>
    ...
    @if ($showConfirmModal)
        <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-zinc-800">
                <h3 class="text-lg font-semibold">Konfirmasi Retur Pembelian?</h3>
                <p class="mt-2 text-sm text-gray-500">Stok akan langsung dikurangi sesuai jumlah barang yang
                    dikembalikan.</p>
