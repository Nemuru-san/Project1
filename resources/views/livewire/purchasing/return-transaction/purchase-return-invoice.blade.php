        ...
        <table class="w-full min-w-[1050px] text-left text-sm">
            <thead class="bg-gray-100 text-xs uppercase dark:bg-zinc-800">
                <tr>
                    ...
                    <th class="px-4 py-3">Retur Pembelian</th>
                    <th class="px-4 py-3">Faktur Pembelian</th>
            ...
            <tbody class="divide-y dark:divide-zinc-700">
                @forelse($invoices as $i=>$invoice)
                    <tr>
                        ...
                        <td class="px-4 py-3 font-mono">{{ $invoice->purchaseReturn?->return_no }}</td>
                        <td class="px-4 py-3 font-mono">{{ $invoice->purchaseInvoice?->code }}</td>
                            ...
                            <div class="inline-block" x-data="{ open: false, top: 0, left: 0, toggle(el) { const r = el.getBoundingClientRect();
                                ...
                                <div x-cloak x-show="open" :style="`position:fixed;top:${top}px;left:${left}px`"
                                    class="z-50 w-52 rounded bg-white py-1 text-left shadow dark:bg-gray-700"><button
                                        ...
                                        href="{{ route('purchases.return.purchase-return-invoice.print', $invoice->id) }}"
        ...
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4">
            <div class="my-4 w-full max-w-3xl rounded-2xl bg-white shadow-2xl dark:bg-zinc-900">
                <div class="flex justify-between border-b px-6 py-4">
                    <div>
                        <h2 class="text-xl font-bold">Tambah Faktur Retur Pembelian</h2>
                        <p class="text-sm text-gray-400">Satu Retur Pembelian hanya dapat memiliki satu faktur retur.
                        </p>
                ...
                <div class="space-y-5 p-6">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div><label class="mb-1 block text-sm">Nomor Kredit Supplier</label><input
                                wire:model="supplierCreditNo"
                                class="w-full rounded-lg border px-3 py-2.5 dark:bg-zinc-800"></div>
                        <div><label class="mb-1 block text-sm">Retur Pembelian</label><select
                                wire:model.live="purchaseReturnId"
                                ...
                                @foreach ($purchaseReturns as $return)
                                    <option value="{{ $return->id }}">{{ $return->return_no }} —
                                        {{ $return->supplier?->name }}</option>
                                @endforeach
                            </select>
                            @error('purchaseReturnId')
                                <p class="text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div><label class="mb-1 block text-sm">Faktur Pembelian</label><select
                                wire:model="purchaseInvoiceId"
                                class="w-full rounded-lg border px-3 py-2.5 dark:bg-zinc-800">
                                <option value="">Pilih Faktur</option>
                                @foreach ($purchaseInvoices as $pi)
                                    <option value="{{ $pi->id }}">{{ $pi->code }} — Sisa Rp
                                        {{ number_format($pi->remaining_amount, 0, ',', '.') }}</option>
                                @endforeach
                            </select>
                            @error('purchaseInvoiceId')
                                <p class="text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
    ...
    @if ($showDetail && $selectedInvoice)
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4">
            <div class="my-4 w-full max-w-4xl rounded-2xl bg-white shadow-2xl dark:bg-zinc-900">
                <div class="flex justify-between border-b px-6 py-4">
                    <div>
                        <h2 class="text-xl font-bold">Detail Faktur Retur Pembelian</h2>
                ...
                <div class="space-y-5 p-6">
                    <div class="grid gap-3 md:grid-cols-3">
                        <div><span class="text-sm text-gray-400">Supplier</span>
                            <p>{{ $selectedInvoice->supplier?->name }}</p>
                        </div>
                        <div><span class="text-sm text-gray-400">Faktur Pembelian</span>
                            <p class="font-mono">{{ $selectedInvoice->purchaseInvoice?->code }}</p>
                        </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                @foreach ($selectedInvoice->purchaseReturn->items as $item)
                                    <tr class="border-t">
    ...
    @if ($showPostModal)
        <div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-zinc-800">
                ...
                <p class="mt-2 text-sm text-gray-500">Sisa utang supplier akan dikurangi dan jurnal pembalik dibuat.
