...
<body><button onclick="window.print()">Cetak</button>
    <h1>FAKTUR RETUR PEMBELIAN</h1>
    <div class="meta">
        <div><b>Nomor Kredit:</b> {{ $invoice->credit_note_no }}</div>
        <div><b>Tanggal:</b> {{ $invoice->invoice_date?->format('d/m/Y') }}</div>
        <div><b>Nomor Supplier:</b> {{ $invoice->supplier_credit_no ?: '-' }}</div>
        <div><b>Supplier:</b> {{ $invoice->supplier?->name }}</div>
        <div><b>Faktur Pembelian:</b> {{ $invoice->purchaseInvoice?->code }}</div>
        <div><b>Retur:</b> {{ $invoice->purchaseReturn?->return_no }}</div>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($invoice->purchaseReturn->items as $item)
                <tr>
